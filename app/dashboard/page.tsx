...
function SkeletonCard() {
  return (
    <div className="animate-pulse rounded-xl border border-cn-border bg-cn-s1 p-5">
      <div className="mb-4 h-10 w-10 rounded-lg bg-cn-hover" />
      <div className="h-7 w-20 rounded bg-cn-hover" />
      <div className="mt-2 h-4 w-28 rounded bg-cn-hover" />
    </div>
  );
}
...
export default function OverviewPage() {
  ...
  const { quotas, totalUsed, totalLimit, totalFree, isLoading: storageLoading } = useStorage();
  const stats: ComputedStats = useStats(files, quotas);
  const loading = storageLoading || filesLoading;
